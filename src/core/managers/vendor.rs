...
use super::dependency::RepositoryContext;
...
impl<'a> VendorManager<'a> {
    ...
    pub fn install(&mut self) -> Result<()> {
        ...
        for dep in &self.spec.deps {
            let path_factory = &RepositoryPathFactory::new(dep, self.cache);
            let path_iterator = &WalkdirPathIterator::new(&self.cache);
            let context = &RepositoryContext::new(path_factory, path_iterator, &self.git_ops);

            let locked_dep = self.lock.find_dep(&dep.url);
            let manager = DependencyManager::new(self.spec, dep, locked_dep, context);
    ...
    pub fn update(&mut self) -> Result<()> {
        ...
        for dep in &self.spec.deps {
            ...
            let path_iterator = &WalkdirPathIterator::new(&self.cache);
            let context = &RepositoryContext::new(path_factory, path_iterator, &self.git_ops);
            let manager = DependencyManager::new(self.spec, dep, None, context);
